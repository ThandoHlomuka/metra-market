...
// Product Data — loaded from admin-managed localStorage
let products = [];
...
function processCheckout(event) {
    ...
    // Create shipment on Bobgo dashboard (after order is saved)
    createBobgoShipment(order);
}
...
/**
 * Create shipment on Bobgo dashboard via serverless proxy
 * This ensures orders appear on Bobgo's dashboard after checkout
 */
function createBobgoShipment(order) {
    // Calculate total parcel dimensions from cart items
    let totalWeight = 0, maxLength = 0, maxWidth = 0, totalHeight = 0, totalValue = 0;
    order.items.forEach(item => {
        ...
        totalHeight += (item.height || 15) * (item.quantity || 1);
    ...
    const shipmentData = {
        orderId: order.id,
        orderNumber: order.id,
        invoiceNumber: order.invoiceNumber || 'INV-' + Date.now(),
        
        // Sender/Shop details (origin)
        senderName: 'Metra Market',
        senderPhone: '+27111234567',
        senderEmail: 'support@metramarket.co.za',
        senderAddress: '1335 Ingwayuma Street',
        senderCity: 'Soweto',
        senderProvince: 'Gauteng',
        senderPostalCode: '1818',
    ...
    fetch('/api/bobgo-create-shipment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(r => r.json())
    .then(data => {
        console.log('Bobgo shipment response:', data);
        if (data.shipmentCreated && data.shipment) {
            // Update order with Bobgo tracking info
            order.bobgoShipment = data.shipment;
